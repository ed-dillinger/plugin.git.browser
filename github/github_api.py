...
from commoncore.baseapi import (
    DB_CACHABLE_API as CACHABLE_API,
    EXPIRE_TIMES,
    connectionException,
    responseException,
)
...
class GitHubAPI(CACHABLE_API):
    ...
    def handel_error(self, error, response, request_args, request_kwargs):
        if response.status_code == 401:
            ...
            kodi.handel_error(
                "Result count exceeds API limit.",
    ...
    def process_response(
        self, url, response, cache_limit, request_args, request_kwargs
    ):
...
def get_download_url(full_name, path):
    ...
    response = requests.get(
        "https://api.github.com/repos/%s/branches" % full_name
    ).json()
...
def sort_results(results, limit=False):
    ...
    if limit:
        return highest_versions(
            sorted(results, key=lambda x: sort_results(x["name"]), reverse=True)
        )
    else:
...
def search(q, method=False):
    if method == "user":
        return GH.request(
            "/search/repositories",
            query={"per_page": page_limit, "q": "user:%s" % q},
            cache_limit=EXPIRE_TIMES.HOUR,
        )
    elif method == "title":
        return GH.request(
            "/search/repositories",
            query={"per_page": page_limit, "q": "in:name %s" % q},
            cache_limit=EXPIRE_TIMES.HOUR,
        )
    elif method == "id":
        results = []
...
def find_zips(user, repo=None):
    ...
    if repo is None:
        results = limit_versions(
            GH.request(
                "/search/code",
                query={"per_page": page_limit, "q": "user:%s filename:%s" % (user, q)},
                cache_limit=EXPIRE_TIMES.HOUR,
            )
        )
    else:
...
def find_zip(user, addon_id):
    ...
    response = GH.request(
        "/search/code",
        query={"q": "user:%s filename:%s*.zip" % (user, addon_id)},
        cache_limit=EXPIRE_TIMES.HOUR,
    )
    if response is None:
...
def get_sha(full_name, url):
    try:
        ...
        class GHC(GitHubAPI):
            ...
            def request(
                ...
                try:
                    ...
                    self.handel_error(
                        connectionException(e), None, request_args, request_kwargs
                    )
                if (
                    ...
                    return self.handel_error(
                        responseException(response.status_code),
                        response,
                        request_args,
                        request_kwargs,
                    )
        ...
        if match:
            path = match.group(1)
            uri = "/repos/{full_name}/contents/{path}".format(
                full_name=full_name, path=path
            )
