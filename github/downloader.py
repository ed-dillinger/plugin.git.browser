...
from .github_api import get_version_by_name, get_version_by_xml, get_sha
...
def download(
    url, full_name, addon_id, destination, unzip=False, quiet=False, verify_hash=True
):
    version = None
        ...
        if unzip:
            ...
            try:
                xml = kodi.vfs.read_file(
                    kodi.vfs.join(destination, kodi.vfs.join(addon_id, "addon.xml")),
                    soup=True,
                )
                ...
                kodi.log(
                    "Unable to fine version from addon.xml for addon: %s" % addon_id
                )
        else:
