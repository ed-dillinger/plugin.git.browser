...
class GitHub_Installer:
    ...
    def __init__(
        ...
        if v >= 17:
            response = kodi.kodi_json_request(
                "Addons.GetAddons",
                {"installed": False, "properties": ["path", "dependencies"]},
            )
            ...
            with dbapi2.connect(dbf) as dbh:
                dbc = dbh.cursor()
                dbc.execute(
                    "SELECT addon.addonID, broken.addonID is Null AS enabled, addon.path FROM addon LEFT JOIN broken on addon.addonID=broken.addonID WHERE enabled=1"
                )
    ...
    def build_dependency_list(self, addon_id, url, full_name, master):
        ...
        if master:
            ...
            xml_str = github.find_xml(full_name)
            self.sources[addon_id]["version"] = github.get_version_by_xml(
                BeautifulSoup(xml_str)
            )
        else:
            version = downloader.download(
                url, full_name, addon_id, self._destination, True, self.quiet
            )
        ...
        for dep in xml.findAll("import"):
            ...
            try:
                if dep["optional"].lower() == "true":
                    if kodi.get_setting("install_optional") == "false":
                        ...
                        c = kodi.dialog_confirm(
                            "Install Optional Dependency", dep["name"], dep["addon"]
                        )
                        if not c:
                            continue
            except:
                pass
            if (
                test in ["xbmc.python", "xbmc.gui"]
                or kodi.get_condition_visiblity("System.HasAddon(%s)" % test) == 1
                or test in self.installed_list
            ):
        ...
        def github_resolver(unmet):
            ...
            c = kodi.dialog_select(
                "GitHub Search Results for %s" % unmet,
        ...
        for unmet in self.unmet_addons:
            ...
            if kodi.get_setting("source_tva") == "true":
                if user_resolver(tva_user, unmet):
                    continue
        ...
        if len(self.unmet_addons):
            ...
            if not self.quiet:
                ...
                kodi.raise_error(
                    ...
                    ",".join(self.unmet_addons),
                )
            kodi.log(
                "Unmet Dependencies for addon install: %s" % addon_id
            )  # % self.addon_id)
    ...
    def enable_addon(self, addon_id):
        try:
            if (
                addon_id in ["xbmc.python", "xbmc.gui"]
                or kodi.get_condition_visiblity("System.HasAddon(%s)" % addon_id) == 1
            ):
                return True
            kodi.log("Enable Addon: %s" % addon_id)
            kodi.kodi_json_request(
                "Addons.SetAddonEnabled", {"addonid": addon_id, "enabled": True}
            )
        except:
            pass
