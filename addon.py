    ...
    kodi.add_menu_item(
        {
            "mode": "search_menu",
            "type": "repository",
            "title": "Search by GitHub Repository Title",
        },
        {"title": "Search by GitHub Repository Title [COLOR red](Advanced)[/COLOR]"},
        icon="repository.png",
    )
    kodi.add_menu_item(
        {"mode": "search_menu", "type": "addonid", "title": "Search by Addon ID"},
        {"title": "Search by Addon ID [COLOR red](Advanced)[/COLOR]"},
        icon="addonid.png",
    )
    kodi.add_menu_item(
        {"mode": "feed_menu"}, {"title": "Search Feeds"}, icon="search_feeds.png"
    )
    kodi.add_menu_item(
        {"mode": "installer_menu"},
        {"title": "Batch Installers"},
        icon="batch_installer.png",
    )
    kodi.add_menu_item(
        {"mode": "about"}, {"title": "About GitHub Installer"}, icon="about.png"
    )
    kodi.add_menu_item(
        {"mode": "settings_menu"}, {"title": "Tools and Settings"}, icon="settings.png"
    )
...
def settings_menu():
    kodi.add_menu_item(
        {"mode": "dependency_search"},
        {"title": "Search for missing dependencies"},
        icon="search_failed.png",
    )
    kodi.add_menu_item(
        {"mode": "update_addons"},
        {"title": "Check for Updates [COLOR red](Advanced)[/COLOR]"},
        icon="update.png",
        visible=kodi.get_setting("enable_updates") == "true",
    )
    kodi.add_menu_item(
        {"mode": "addon_settings"}, {"title": "Addon Settings"}, icon="settings.png"
    )
...
def search_menu():
    ...
    kodi.add_menu_item(
        {"mode": "void"},
        {"title": "[COLOR darkorange]%s[/COLOR]" % kodi.arg("title")},
        icon="null",
        menu=menu,
    )
    kodi.add_menu_item(
        {"mode": "search", "type": kodi.arg("type")},
        {"title": "*** New Search ***"},
        icon="null",
        menu=menu,
    )
    results = DB.query_assoc(
        "SELECT search_id, query FROM search_history WHERE search_type=? ORDER BY ts DESC LIMIT 25",
        [kodi.arg("type")],
        quiet=True,
    )
    if results is not None:
        for result in results:
            ...
            menu.add(
                ...
                {"mode": "history_delete", "id": result["search_id"]},
            )
            kodi.add_menu_item(
                {"mode": "search", "type": kodi.arg("type"), "query": result["query"]},
                {"title": result["query"]},
                menu=menu,
                icon="null",
            )
...
def dependency_search():
    results = DB.query(
        "SELECT addon_id FROM failed_depends WHERE resolved=0 ORDER BY addon_id ASC"
    )
    if results is not None:
        for result in results:
            if kodi.has_addon(result[0]):
                DB.execute(
                    "UPDATE failed_depends SET resolved=1 WHERE addon_id=?", [result[0]]
                )
                DB.commit()
                continue
            kodi.add_menu_item(
                {"mode": "search", "type": "addonid", "query": result[0]},
                {"title": result[0]},
                icon="addonid.png",
            )
...
def search():
    ...
    def username():
        ...
        for r in github.sort_results(response["items"]):
            ...
            if r["is_feed"]:
                ...
                kodi.add_menu_item(
                    {"mode": "install_feed", "url": url},
                    {"title": r["name"], "display": r["display"]},
                    menu=menu,
                    icon="null",
                )
            elif r["is_installer"]:
    ...
    def addonid():
        ...
        for i in results:
            ...
            menu.add(
                "Search Username",
                {"mode": "search", "type": "username", "query": r["owner"]["login"]},
            )
            kodi.add_menu_item(
                {
                    "mode": "github_install",
...
def feed_menu():
    kodi.add_menu_item(
        {"mode": "install_local_feed"},
        {"title": "*** Local Search Feed File ***"},
        icon="install_feed_local.png",
    )
    ...
    for feed in feeds:
        ...
        menu.add(
            "Delete Feed",
            {"mode": "delete_feed", "title": title, "id": feed["feed_id"]},
        )
        kodi.add_menu_item(
            {"mode": "list_feed", "url": feed["url"]},
            {"title": title},
            menu=menu,
            icon="null",
        )
...
def installer_menu():
    kodi.add_menu_item(
        {"mode": "browse_local"},
        {"title": "*** Install From Local File ***"},
        icon="install_batch_local.png",
    )
...
def install_feed():
    if kodi.mode == "install_feed":
        ...
        url = kodi.dialog_browser(
            "Select a feed file", type=kodi.BROWSER_TYPES.FILE, mask=".zip"
        )
    ...
    try:
        for f in xml.findAll("feeds"):
            ...
            url = f.find("url").text
            DB.execute(
                "INSERT INTO feed_subscriptions(name, url) VALUES(?,?)", [name, url]
            )
        DB.commit()
...
def install_batch():
    ...
    if kodi.mode == "install_batch":
        ...
        url = kodi.dialog_browser(
            "Select a install file", type=kodi.BROWSER_TYPES.FILE, mask=".zip"
        )
    ...
    if not kodi.dialog_confirm(
        ...
        "This will install a list of addons.",
    ...
    for a in xml.findAll("addon"):
        ...
        if not kodi.has_addon(addon_id):
            ...
            if url:
                installed_list += github_installer.GitHub_Installer(
                    addon_id,
                    url,
                    full_name,
    ...
    if configs is not None and "dir" in configs.attrs[0]:
        ...
        for config in configs.findAll("config"):
            ...
            kodi.vfs.write_file(
                kodi.vfs.join(destination, source),
                zip_ref.read(config_dir + "/" + source),
            )
    ...
    if len(failed_list):
        kodi.dialog_ok(
            "Batch Error", "One or more Addons failed to install", "See log for list"
        )
...
def delete_feed():
    if not kodi.dialog_confirm(
        "Delete Feed?", kodi.arg("title"), "Click YES to proceed."
    ):
        return
...
def feed_list():
    ...
    try:
        ...
        for r in xml.iter("repository"):
            ...
            title = "%s: %s" % (name, desc)
            kodi.add_menu_item(
                {"mode": "search", "type": "username", "query": username},
                {"title": title, "plot": desc},
                icon="null",
            )
        kodi.eod()
...
def github_install():
    ...
    c = kodi.dialog_confirm(
        "Confirm Install", kodi.arg("file"), yes="Install", no="Cancel"
    )
    if not c:
        return
...
def about():
    ...
    path = kodi.vfs.join(
        kodi.get_path(), "resources/language/%s/github_help.txt", KODI_LANGUAGE
    )
    if not kodi.vfs.exists(path):
        path = kodi.vfs.join(
            kodi.get_path(), "resources/language/English/github_help.txt"
        )
...
def update_addons():
    ...
    if not quiet:
        c = kodi.dialog_confirm(
            "Confirm Update", "Check for updates", yes="Update", no="Cancel"
        )
        if not c:
            return
