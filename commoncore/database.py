...
class BASEDatabase:
    ...
    __connected = False
    __ignore_errors = [
        re.compile("column (.)+ is not unique$", re.IGNORECASE),
        re.compile("UNIQUE constraint failed", re.IGNORECASE),
        re.compile("no such table: version", re.IGNORECASE),
    ]
    autoconnect = True
    ...
    def do_init(self):
        ...
        try:
            test = self.query(
                "SELECT 1 FROM version WHERE db_version >= ?",
                [self.db_version],
