...
class BASE_API:
    ...
    def generate_user_agent(self):
        ...
        user_agent = RAND_UAS[index].format(
    ...
    def request(
        ...
        if response.status_code == requests.codes.ok or response.status_code == 201:
            return self.process_response(
                url, response, cache_limit, request_args, request_kwargs
            )
        else:
            return self.handel_error(
                responseException(response.status_code),
...
class DB_CACHABLE_API(CACHABLE_API):
        ...
        if DB_Object is None:
            ...
            def cache_response(url, response, cache_limit):
                if cache_limit == 0:
                    return False
                self.DB.execute(
                    "REPLACE INTO request_cache(url, response) VALUES(?,?)",
                    [url, response],
                )
                self.DB.commit()
            ...
            def get_cached_response(url, cache_limit):
                ...
                if cache_limit == -1:
                    results = self.DB.query(
                        "SELECT response FROM cached_requests WHERE url=?",
                        [url],
                    ...
                    results = self.DB.query(
                        "SELECT response FROM cached_requests WHERE age < ? AND url=?",
                        [cache_limit, url],
                        force_double_array=False,
                    )
                if results:
    ...
    def get_cached_response(self, url, cache_limit):
        ...
        with self.db_lock:
            if cache_limit == -1:
                ...
                if cache_limit():
                    self.execute(
                        "SELECT response FROM cached_requests WHERE url=?", [url]
                    )
                    self.db_lock.release()
                else:
                    return False
    ...
    def cache_response(self, url, response, cache_limit):
        ...
        with self.db_lock:
            self.execute(
                "REPLACE INTO request_cache(url, response) VALUES(?,?)", [url, response]
            )
            self.commit()
