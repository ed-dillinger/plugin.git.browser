...
try:
    ...
    BASE_FANART_URL = "%s://%s:%s/api/images" % (
        client_protocol,
        client_host,
        client_port,
    )
...
DEFAULT_VIEWS = enum(
    ...
    LIST=int(get_setting("default_list_view"))
    if get_setting("default_list_view")
    else DEFAULT_VIEW,
    FOLDER=int(get_setting("default_folder_view"))
    if get_setting("default_folder_view")
    else DEFAULT_VIEW,
    MOVIES=int(get_setting("default_movie_view"))
    if get_setting("default_movie_view")
    else DEFAULT_VIEW,
    SHOWS=int(get_setting("default_show_view"))
    if get_setting("default_show_view")
    else DEFAULT_VIEW,
    SEASONS=int(get_setting("default_season_view"))
    if get_setting("default_season_view")
    else DEFAULT_VIEW,
    EPISODES=int(get_setting("default_episode_view"))
    if get_setting("default_episode_view")
    else DEFAULT_VIEW,
    STREAMS=int(get_setting("default_stream_view"))
    if get_setting("default_stream_view")
    else DEFAULT_VIEW,
    GAMES=int(get_setting("default_game_view"))
    if get_setting("default_game_view")
    else DEFAULT_VIEW,
)
