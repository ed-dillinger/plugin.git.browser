...
def dialog_select(heading, options, delay=0, preselect=-1, detailed=False):
    dialog = xbmcgui.Dialog()
    index = dialog.select(
        heading, options, autoclose=delay, preselect=preselect, useDetails=detailed
    )
    if index >= 0:
        return index
    else:
...
def dialog_browser(
    ...
    return xbmcgui.Dialog().browse(
        type, heading, shares, mask, thumbs, force_folder, default, multiple
    )
